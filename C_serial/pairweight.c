 ...
 * function w = pair_weight2(patch1, patch2, pars, option)
	...
	w = w1*w2;
 */
void diff_square(int patch0, double *data, double *temp, double *pos,
                 double *ptemp, int scale, int pos_dim, int n_patch);
void reduce(int patch0, double *w, double *temp, double *ptemp, int scale,
            int pos_dim, int par0, int par1, int n_patch);

void pairweight(double *w, int n_patch, double *data, double *pos,
                int scale[2], int pos_dim, int par[2], int option)
{
	...
	for (j=1; j<n_patch; j++){
		diff_square(j, data, temp, pos, ptemp, scale[0]*scale[1], pos_dim, 
                    n_patch);
		reduce(j, w, temp, ptemp, scale[0] * scale[1], pos_dim, par[0],
               par[1], n_patch);
	}
                 ...
                 double *ptemp, int scale, int pos_dim, int n_patch)
{
	...
	for (patch1=0; patch1<patch0; patch1++){
		for (i=0; i<scale; i++){
			temp[patch1 * scale + i] =
                (data[patch1 * scale + i] - data[patch0 * scale + i]) *
                (data[patch1 * scale + i] - data[patch0 * scale + i]);
		}
		for (i=0; i<pos_dim; i++){
			ptemp[patch1 * pos_dim + i] =
                (pos[patch1 * pos_dim + i] - pos[patch0 * pos_dim + i]) *
                (pos[patch1 * pos_dim + i] - pos[patch0 * pos_dim + i]);
		}
	}
}
